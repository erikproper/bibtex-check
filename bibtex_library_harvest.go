...
func (l *TBibTeXLibrary) harvestFindDblpCandidates(e TBibTeXEntry) string {
	...
	// Repeat the source entry right above the candidate list, in the same rendering
	// used for the initial "Source entry:" display — not a separate ad-hoc summary.
	// By this point in the pipeline (after the key-match and library-title-match
